...
本副本 compose 校准过程（两轮）：
- **v4（源码推）**：细颗粒、微弱闪烁、屏内常驻玻璃层（凸面+圆角边框+边缘压暗）。
- **v5（连拍实测纠正 v4 的过度解读）**：实测参考站**屏内页面全出血——没有常驻边框**，凸面
  几乎无感；其 rounded SDF 属于**翻页过渡件**（`SampleComposeTexture`/`exitCurlProgress`），
  不是视口常驻效果。已改为：屏内全出血近平面，凸面+圆角边框只在穿屏一瞬快闪
  （`uGlass·uGlassFlash`，0.46→0.60 快进快出）；颗粒再降（0.05/0.035）；穿幕软闪深度 0.3；
  屏内扫描线减半。外部世界黑白为 iiiiiei 自有方向（用户指定），参考站外部是彩色办公室。
